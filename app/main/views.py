from flask import abort, current_app, flash, render_template, request, redirect, url_for
...
def edit(id):
    post_ = Post.query.get_or_404(id)
    if current_user != post_.author and \
...
def follow(username):
    ...
    if user_ is None:
        flash("Invalid user.")
        return redirect(url_for(".index"))
    if current_user.is_following(user_):
...
def unfollow(username):
    ...
    if user_ is None:
        flash("Invalid user.")
        return redirect(url_for(".index"))
    if not current_user.is_following(user_):
...
def followers(username):
    ...
    if user_ is None:
        ...
        return redirect(url_for(".index"))
    page = request.args.get("page", 1, type=int)
    pagination = user_.followers.paginate(
        page=page,
